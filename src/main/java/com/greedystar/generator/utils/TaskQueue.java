...
public class TaskQueue {
    ...
    private void initCommonTasks(String className) {
        ...
        if (!StringUtil.isBlank(ConfigUtil.getConfiguration().getPath().getVo())) {
            taskQueue.add(new VoTask(className));
        }
        if (!StringUtil.isBlank(ConfigUtil.getConfiguration().getPath().getReq())) {
            taskQueue.add(new ReqTask(className));
        }
    }
